...
│
├── solution.py            # Main script - run to create predictions
│
│   ── Files you implement ──────────────────────────────────────────────
...
**Feature Engineering & Dimensionality Reduction**: Applicants are encouraged to experiment with adding hand-crafted features during the aggregation step, drawing on geometrical or topological methods.
...
Results are averaged across folds (if using k-fold) and saved to `results.json`.
...
**Q2:** What the applicants must include in the repository?
...
- Reproducibility instructions: exact commands to run your solution and acquire the same `predictions.csv`
- Final solution description: What components you modified? What your final approach is? Why you made these choices?
- Experiments and failed attempts: What ideas you tried but did not include in the final solution?

**Q4:** Reproducibility
**A4:** The repository must be self-contained and runnable with the provided `solution.py` file.
